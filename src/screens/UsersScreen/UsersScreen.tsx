...
interface UserFeature { id: string; label: string; enabled: boolean; }
interface AILimit { id: string; label: string; value: number; max: number; }
interface User { id: string; username: string; email: string; role: UserRole; features: UserFeature[]; aiLimits: AILimit[]; createdAt: string; status: 'active' | 'inactive'; }
interface Notice { type: 'success' | 'error' | 'warning'; message: string; }
...
const FEATURE_LABELS: Record<string, string> = { f1: 'Exportar SVG', f2: 'Exportar PNG', f3: 'Templates', f4: 'API Access' };
const AI_LIMIT_LABELS: Record<string, string> = { l1: 'Generaciones/mes', l2: 'Tokens/mes' };
...
interface UserCardProps { user: User; isExpanded: boolean; onToggleExpand: () => void; onEdit: () => void; onDelete: () => void; }

function UserCard({ user, isExpanded, onToggleExpand, onEdit, onDelete }: UserCardProps) {
  const roleColors: Record<UserRole, string> = { Admin: 'bg-accent-muted/70 text-accent', Editor: 'bg-warning-muted/70 text-warning', Usuario: 'bg-surface-muted/70 text-content-secondary' };
  return (
    <div className={cn('group relative rounded-xl border border-border bg-surface transition-all duration-200', 'hover:border-accent-muted hover:shadow-card-hover', isExpanded && 'border-accent shadow-panel-lg')}>
      ...
      {isExpanded && (
        <div className="border-t border-border bg-surface-muted/50 p-5 animate-fade-in">
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <h4 className="mb-3 text-xs font-semibold tracking-wide text-content-muted uppercase">Features activos</h4>
              <div className="flex flex-wrap gap-2">
                {user.features.map((feature) => (
                  <span key={feature.id} className={cn('inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium', feature.enabled ? 'bg-success-muted/70 text-success' : 'bg-surface-muted/70 text-content-muted')}>
                    {feature.enabled && <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}{feature.label}
                  </span>
                ))}
              </div>
            </div>
                ...
                {user.aiLimits.map((limit) => (
                  <div key={limit.id}>
                    ...
                    <div className="h-2 overflow-hidden rounded-full bg-surface-muted">
                      <div className={cn('h-full rounded-full transition-all duration-500', (limit.value / limit.max) > 0.8 ? 'bg-danger' : (limit.value / limit.max) > 0.5 ? 'bg-warning' : 'bg-success')} style={{ width: `${(limit.value / limit.max) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="mt-4 flex items-center gap-4 border-t border-border/50 pt-4 text-xs text-content-muted">
            <span>Creado: {new Date(user.createdAt).toLocaleDateString('es-ES')}</span><span>•</span><span>ID: {user.id}</span>
          </div>
        </div>
      )}
    </div>
...
interface InlineEditPanelProps { user: User; onSave: (user: User) => void; onCancel: () => void; }
...
function InlineEditPanel({ user, onSave, onCancel }: InlineEditPanelProps) {
  ...
  return (
    <div className="rounded-xl border border-accent bg-surface-elevated p-7 shadow-panel-lg animate-scale-in">
      ...
      <div className="mb-6 space-y-4">
        <div><label className="mb-2 block text-xs font-medium text-content-muted">Presets rápidos</label><div className="flex flex-wrap gap-2">
          {(['Usuario', 'Editor', 'Admin'] as UserRole[]).map((role) => (
            <button key={role} onClick={() => handlePresetSelect(role)} className={cn('rounded-full px-4 py-1.5 text-xs font-medium transition-all duration-200', selectedRole === role ? 'bg-accent text-accent-foreground shadow-glow' : 'bg-surface-muted/70 text-content-secondary hover:bg-surface-elevated hover:text-content')} >{role}</button>
          ))}
        </div></div>
        <div><label className="block text-xs font-medium text-content-muted">Features individuales</label><div className="space-y-2 rounded-xl border border-border bg-surface-muted/30 p-3">
          {editedUser.features.map((feature) => (
            <div key={feature.id} className="flex items-center justify-between"><span className="text-sm text-content">{feature.label}</span><Switch checked={feature.enabled} onChange={() => handleFeatureToggle(feature.id)} /></div>
          ))}
        </div></div>
      </div>
        ...
        <div className="space-y-4 rounded-xl border border-border bg-surface-muted/30 p-4">
          {editedUser.aiLimits.map((limit) => (
            <div key={limit.id}>
              ...
              <input type="range" min={0} max={limit.max} value={limit.value} onChange={(e) => handleAILimitChange(limit.id, parseInt(e.target.value, 10))} className="h-2 w-full appearance-none rounded-full bg-surface-muted cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-accent [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:shadow-panel [&::-webkit-slider-thumb]:transition-transform [&::-webkit-slider-thumb]:hover:scale-110 [&::-moz-range-thumb]:h-4 [&::-moz-range-thumb]:w-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-accent [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:cursor-pointer" style={{ background: `linear-gradient(to right, rgb(var(--accent)) 0%, rgb(var(--accent)) ${(limit.value / limit.max) * 100}%, rgb(var(--surface-muted)) ${(limit.value / limit.max) * 100}%, rgb(var(--surface-muted)) 100%)` }} />
              <div className="mt-2 flex items-center gap-2"><Input type="number" value={limit.value} onChange={(e) => handleAILimitChange(limit.id, parseInt(e.target.value || '0', 10))} className="h-8 w-28 text-xs" /><span className="text-xs text-content-muted">/ {limit.max.toLocaleString()}</span></div>
            </div>
          ))}
        </div>
      </div>
...
export function UsersScreen() {
  ...
  return (
    <PageShell
      title="Usuarios"
      description="Gestiona usuarios, permisos y configuraciones de acceso al sistema."
      actions={<Button><svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>Nuevo usuario</Button>}
    >
      <div className="mb-8 flex flex-wrap items-center gap-4">
        ...
        <Select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value as UserRole | 'all')} className="w-40">
          <option value="all">Todos los roles</option><option value="Usuario">Usuario</option><option value="Editor">Editor</option><option value="Admin">Admin</option>
        </Select>
      </div>
      ...
      {!isLoading && filteredUsers.length > 0 && (
        <div className="mt-6 flex items-center justify-between border-t border-border pt-6 text-sm text-content-muted">
