...
function SearchInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div className="relative">
      ...
      <input
        type="text"
        placeholder="Buscar especificaciones Vega..."
        value={value}
    ...
    <button
      ...
      className={cn(
        ...
        selected
          ...
          : 'border-border/60 bg-surface hover:border-accent-muted hover:shadow-card-hover hover:-translate-y-0.5',
      )}
    >
      {/* Preview */}
      ...
      <div className="flex flex-col gap-1 p-5">
        <div className="flex items-start justify-between gap-2">
          <h3 className={cn('text-sm font-medium leading-tight', selected ? 'text-accent' : 'text-content')}>
            {spec.name}
          </h3>
          {selected && (
            <span className="mt-0.5 shrink-0 rounded-full bg-accent/80 px-2 py-0.5 text-[10px] font-semibold text-accent-foreground">
              Activo
            </span>
          )}
        </div>
        ...
        <span className="mt-1 text-[11px] text-content-muted">
          {new Date(spec.lastUpdated).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' })}
        </span>
      </div>
    </button>
  )
...
function DetailPanel({ spec }: { spec: VegaSpec }) {
  return (
    <div className="flex flex-col gap-5 rounded-2xl border border-border/60 bg-surface p-7 animate-fade-in">
      {/* Header */}
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="mb-1 flex items-center gap-2">
            <span className="rounded-md bg-surface-muted px-2 py-0.5 text-[11px] text-content-muted">
              {spec.category}
            </span>
          </div>
      ...
      <div className="relative aspect-[3/2] overflow-hidden rounded-xl bg-surface-muted">
        <img src={spec.preview} alt={spec.name} className="h-full w-full object-cover" />
      </div>

      {/* Metadata */}
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-border/60 bg-surface-muted/30 p-3">
          <p className="text-[11px] text-content-muted">Última actualización</p>
          <p className="mt-0.5 text-sm font-medium text-content">
            {new Date(spec.lastUpdated).toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' })}
          </p>
        </div>
        <div className="rounded-xl border border-border/60 bg-surface-muted/30 p-3">
          <p className="text-[11px] text-content-muted">Categoría</p>
          <p className="mt-0.5 text-sm font-medium text-content">{spec.category}</p>
        </div>
      </div>
...
export function VegaScreen() {
  ...
  return (
    <PageShell
      title="Vega"
      description="Visualizaciones de datos interactivas con Vega. Explora especificaciones, personaliza gráficos y exporta para integrar en tus reportes."
      storageBadge={`${specs.length} specs`}
      actions={
    ...
    >
      {/* Search */}
      <div className="mb-6 max-w-sm animate-slide-up" style={{ animationDelay: '20ms' }}>
      ...
      <div className="flex flex-col gap-10 lg:flex-row">
        ...
        <div className="w-full lg:w-80 xl:w-96">
          ...
          <div className="flex flex-col gap-4">
            {loading
              ...
              : filtered.length === 0
              ? (
                <div className="flex flex-col items-start border-t border-border py-10">
                  <p className="text-base font-medium text-content">Sin resultados</p>
                  <p className="mt-1 text-sm text-content-secondary">No hay specs que coincidan con tu búsqueda.</p>
                </div>
              )
              : filtered.map((spec, i) => (
                <div key={spec.id} className="animate-slide-up" style={{ animationDelay: `${i * 40}ms` }}>
                  <SpecCard
                    spec={spec}
        ...
        <div className="min-w-0 flex-1 lg:sticky lg:top-6 lg:self-start">
          {selectedSpec ? (
            <DetailPanel spec={selectedSpec} />
          ) : (
            <div className="flex flex-col items-start rounded-2xl border border-dashed border-border bg-surface-muted/20 py-16 text-center">
              <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-surface-muted text-content-muted">
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" />
                </svg>
              </div>
              <p className="text-base font-medium text-content">Selecciona una especificación</p>
              <p className="mt-1.5 max-w-xs text-sm text-content-secondary">
                Haz clic en una spec del catálogo para ver su detalle y acciones disponibles.
              </p>
            </div>
          )}
        </div>
